...
// return the path to the file containing the preamble digest
tstring GetPreambleDigestFilePath()
{
    return GetAuxDirPath() + _T("\\") + texbasename + _T("-preamble.md5");
}
...
// Return true if the preamble changed since last compilation
    ...
    if( PreambleType != None ) {
        ...
        if( PreambleType == External ) {
            ...
            if( res == SRC_FRESHER ) {
                ...
                bool success = LoadPreambleDigest(previousDigest);

                if (success && FindInternalPreamble(&preamble_size)) {
                    newDigest.DigestFile(CFilename(sCurrDir, preamble_filename), preamble_size);
                }
                else {
                    success = false;
                }
...
bool FindInternalPreamble(DWORD *preamble_len)
{
...
int cleanup()
{
    ...
    static PCTSTR clean_ext[] = { _T(".toc"), _T(".tdo"), _T(".out"), _T(".not"), _T(".lot"),
                                  ...
                                  _T(".gls"), _T(".glo"),
                                  _T(".aux.bak"), _T(".out.bak"), _T(".pdfsync"), _T("-preamble.dep"), _T("-preamble.md5"),
...
int makeindex(AbortableProcessLauncher &launcher, tstring opt)
{
    ...
    tstring cmdline = tstring(_T("makeindex ")) + opt;
    if( auxdir != _T("") )
        cmdline += auxdir+_T("\\");
    cmdline += texbasename;
    tcout << fgMsg << " Running '" << cmdline << "'\n" << fgLatex;
